...
fn main() {
    ...
    let s = (m + 1) * n;
    let t = s + 1;
    let mut flow = min_cost_flow_library::MinCostFlow::<i64>::new(n * (m + 1) + 2);
    for i in 0..n {
        flow.add_edge(s, i, 1, 0);
    }
    for j in 0..m {
        for k in 0..n {
            flow.add_edge((j + 1) * n + k, t, 1, 0);
            for i in 0..n {
                flow.add_edge(i, (j + 1) * n + k, 1, (k as i64 + 1) * costs[i][j])
            }
        }
    }
    println!(
        "{}",
        flow.min_cost_flow(s, t, n as i64).unwrap() as f64 / n as f64
    );
}
