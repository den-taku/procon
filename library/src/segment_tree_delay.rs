...
pub mod segment_tree_delay_library {
    ...
    #[derive(Debug)]
    pub struct DelaySegmentTree<T, F>
    where
        F: Fn(T, T) -> T,
    {
        tree: Vec<T>,
    ...
    impl<T, F> std::ops::Index<usize> for DelaySegmentTree<T, F>
    where
        F: Fn(T, T) -> T,
    {
        type Output = T;
    ...
    impl<T, F> std::ops::IndexMut<usize> for DelaySegmentTree<T, F>
    where
        F: Fn(T, T) -> T,
    {
        #[inline]
    ...
    mod tests_segment_tree {
        ...
        fn for_rmq1() {
            ...
            let answers = &[1, 2];
            let mut rmq = DelaySegmentTree::new(n, std::cmp::min, std::usize::MAX);
            let mut i = 0;
            for &(d, x, y) in &com {
                match d {
                    0 => rmq.update(x, y),
