...
fn main() {
    ...
    let mut adjacent = vec![std::collections::HashSet::new(); n];
    for (i, &(u, v)) in uv.iter().enumerate() {
        adjacent[u - 1].insert((v - 1, i));
        adjacent[v - 1].insert((u - 1, i));
    }
    let mut parent = vec![None; m];
    for (i, v) in adjacent.iter().enumerate() {
        if v.len() == 1 {
            let edge = v.iter().next().unwrap();
            if parent[edge.1].is_some() {
                println!("0");
                return;
            } else {
                parent[edge.1] = Some(i)
            }
        } else if v.len() == 0 {
            println!("0");
            return;
    ...
    for (i, &(u, v)) in uv.iter().enumerate() {
        if parent[i].is_some() {
            adjacent[u - 1].remove(&(v - 1, i));
            adjacent[v - 1].remove(&(u - 1, i));
        }
    }
    let mut uf_tree = union_find_library::UnionFind::new(n);
    for (i, v) in adjacent.iter().enumerate() {
        for e in v.iter() {
            uf_tree.unite(i, e.0);
        }
    }
    let v = uf_tree.to_vec().iter().filter(|v| v.len() > 1).count();
    println!("{}", pow(2, v));
}
