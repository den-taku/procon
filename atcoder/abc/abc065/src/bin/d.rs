...
fn main() {
    ...
    let mut towns = towns.iter().enumerate().collect::<Vec<_>>();
    towns.sort_by_key(|e| (e.1).0);
    for (a, b) in towns.windows(2).map(|v| (v[0], v[1])) {
        kruskal.add_edge(a.0, b.0, (b.1).0 - (a.1).0);
    }
    towns.sort_by_key(|e| (e.1).1);
    for (a, b) in towns.windows(2).map(|v| (v[0], v[1])) {
        kruskal.add_edge(a.0, b.0, (b.1).1 - (a.1).1);
    }
    println!(
        "{}",
        kruskal
