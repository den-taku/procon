...
fn main() {
    ...
    for i in 0..h {
        for j in 0..w {
            order.push(Reverse((a[i * w + j], i, j)));
        }
    }
    order.sort();
    let mut ans = vec![ModInteger::new(1, MOD); h * w];
    for Reverse((e, i, j)) in order {
        // up
        if i > 0 && e < a[(i - 1) * w + j] {
            let tmp = ans[(i - 1) * w + j];
            ans[i * w + j] += tmp;
        ...
        // right
        if j < w - 1 && e < a[i * w + j + 1] {
            let tmp = ans[i * w + j + 1];
            ans[i * w + j] += tmp;
        }
        // down
        if i < h - 1 && e < a[(i + 1) * w + j] {
            let tmp = ans[(i + 1) * w + j];
            ans[i * w + j] += tmp;
        }
        // left
    ...
    println!(
        "{}",
        ans.into_iter().fold(ModInteger::new(0, MOD), |s, e| s + e)
    );
}
