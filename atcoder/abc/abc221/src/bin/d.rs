...
fn main() {
    ...
    ab.push((0, 1));
    let mut bit_s = bit_sum_library::BitSum::<i64>::new(2 * n + 5);
    let mut bit_t = bit_sum_library::BitSum::<i64>::new(2 * n + 5);
    let map = compress(&ab);
    for (a, b) in ab {
        if a == 0 {
            continue;
    ...
    for i in 0..map.len() - 1 {
        let s = bit_s.sum_in_range(0..i + 1);
        let t = bit_t.sum_in_range(0..i + 1);
        let k = s + t;
        if k > n as i64 {
            continue;
        }
        let start = *map.get(&(i + 1)).unwrap();
        let end = *map.get(&(i)).unwrap();
        let range = start - end;
        ans[k as usize] += range;
    }
    println!(
        "{}",
        ans.iter()
