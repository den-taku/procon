...
fn main() {
    ...
    a.sort();
    let bi = number_library::BinomialCoefficient::new(n, MOD);
    let mut ans = 0i128;
    for (i, &e) in a.iter().enumerate().skip(k - 2) {
        ans += bi.comp(i, k - 1).value() as i128 * e;
        ans %= MOD as i128;
    }
    for (i, &e) in a.iter().take(n - k + 1).enumerate() {
        ans -= bi.comp(n - i - 1, k - 1).value() as i128 * e;
        ans %= MOD as i128;
    }
    println!(
        "{}",
        number_library::ModInteger::new((ans + MOD as i128) as u64, MOD)
    )
}
