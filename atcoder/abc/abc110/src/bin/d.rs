...
fn main() {
    ...
    let mut ans = ModInteger::new(1, MOD);
    let n_c_r = BinomialCoefficient::new(
        n + *factors.iter().map(|(_p, r)| r).max().unwrap() as usize,
        MOD,
    );
    for (&_p, &r) in factors.iter() {
        ans *= n_c_r.comp(r as usize + n - 1, r as usize);
    }
    println!("{}", ans);
}
