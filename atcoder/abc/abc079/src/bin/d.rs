...
fn main() {
    ...
    let mut dijkstra = dijkstra_library::Dijkstra::new(10);
    for i in 0..10 {
        for j in 0..10 {
            dijkstra.add_edge(j, i, c[i * 10 + j])
        }
    }
    let distance = dijkstra.shortest_path(1);
    println!(
        "{}",
        a.iter()
            .map(|&v| if v == -1 { 0 } else { distance[v as usize] })
            .sum::<usize>()
    )
}
