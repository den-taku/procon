...
fn main() {
    let (m, costs, path) = input();
    let sum = path.iter().map(|i| costs[i - 1]).sum::<i64>();
    let s = 0;
    let t = path.len() + 1;
    let mut flow = min_cost_flow_library::MinCostFlow::<i64>::new(t + 1);
    flow.add_edge(0, 1, std::i64::MAX, 0);
...
#[inline]
fn input() -> (usize, Vec<i64>, Vec<usize>) {
    let (m, n, k) = {
        let inp = read_line::<usize>();
        (inp[0], inp[1], inp[2])
    };
    let mut costs = vec![0; n];
    for p in costs.iter_mut() {
        *p = read_line::<i64>()[0];
    }
