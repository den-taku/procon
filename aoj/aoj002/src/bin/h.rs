...
fn main() {
    while let Some((trains, group)) = input() {
        let mut ub = group + 1;
        let mut lb = 0;
        while ub - lb > 1 {
            //
...
#[inline(always)]
fn input() -> Option<(Vec<Vec<(usize, usize, usize)>>, usize)> {
    let n = read_line::<usize>()[0];
    if n == 0 {
        return None;
    }
    let mut trains = vec![Vec::new(); n - 1];
    for i in 0..n - 1 {
        let m = read_line::<usize>()[0];
        for _ in 0..m {
            let e = read_line::<usize>();
            trains[i].push((e[0], e[1], e[2]))
        }
    }
    let g = read_line::<usize>()[0];
    Some((trains, g))
}
