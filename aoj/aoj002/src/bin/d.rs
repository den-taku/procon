...
fn main() {
    let (n, q) = input();
    let mut tree = segment_tree_delay_library::DelayedSegmentTree::init(
        n,
        std::cmp::min,
        std::i32::MAX,
        |a, b, _c| a + b,
    ...
    for query in q {
        match query {
            Add(s, t, x) => tree.act_in_range(s..t + 1, x),
            Find(s, t) => ans.push(tree.find(s..t + 1).to_string()),
        }
    }
    println!("{}", ans.join("\n"))
}
...
fn input() -> (usize, Vec<Query>) {
    ...
    for _ in 0..q {
        let e = read_line::<i32>();
        if e[0] == 0 {
            v.push(Add(e[1] as usize, e[2] as usize, e[3]))
        } else {
            v.push(Find(e[1] as usize, e[2] as usize))
        }
    }
    (n, v)
}
